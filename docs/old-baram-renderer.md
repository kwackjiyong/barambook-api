...
- **빈 여백**: 무대 바탕색(`#efe8d9`)으로 채운다. 프론트가 무대·썸네일·목록 타일에
  모두 같은 색을 써서 화면에서는 바탕에 묻힌다.
- **캐릭터 위**: 밑색을 살짝 밀어 물들인다. 바탕색으로 덮으면 화면에서 그대로 보인다.

캐릭터를 아예 비켜 여백에만 찍으면 아래 몇 줄만 잘라내도 표시가 사라지므로,
글자를 캐릭터 아래쪽에 걸치게 둔다. 바탕색을 바꾸면 여기 값도 같이 바꿔야 한다.
