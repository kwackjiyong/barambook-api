...
글자는 그림 **아래에 띠로 붙이지 않고 캐릭터 위에 겹쳐** 찍는다. 띠는 아래 몇 줄만
잘라내면 원본이 그대로 남아 자동 수집을 전혀 못 막기 때문이다. 위치는 캔버스 바닥이
아니라 실제로 그려진 화소의 아래쪽에 맞춘다(캔버스에는 모든 자세를 담느라 여백이 넓어,
바닥에 찍으면 다시 빈 곳에 놓인다). 폭이 좁아 전체 표기가 안 들어가면 `BB.COM` 으로 줄인다.
