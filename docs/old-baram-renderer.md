...
잘라내면 원본이 그대로 남아 자동 수집을 전혀 못 막기 때문이다.

- **자리**: 캔버스가 아니라 그려진 화소 기준. 글자를 얹을 줄에서 끊기지 않고 이어지는
  가장 긴 구간(옷자락)을 찾아 거기에 맞춘다. 바운딩 박스를 쓰면 무기·방패까지 폭에 들어가
  글자가 몸통보다 넓어지고, 빈 곳에 떨어진 부분이 빠져 조각만 남는다.
- **크기**: 몸통 폭에 비례해 키운다. 확대가 끝난 그림에 1픽셀로 찍으면 확대율 1과 8 사이에서
  상대 크기가 여덟 배 벌어진다. 폭에 안 들어가면 `BB.COM`, `BB` 순으로 줄인다.
- **색**: 먹색 글자를 얹으면 스티커처럼 뜬다. 대신 밑에 깔린 색을 어둡게(어두운 색이면 밝게)
  밀어 그림이 가진 색조를 따라가게 한다. 빈 화소는 건드리지 않아 실루엣이 바뀌지 않는다.

진하기는 `TINT_FACTOR` 하나로 조절한다. 지금 값(0.7)은 그림을 해치지 않는 쪽이라
작은 썸네일에서는 거의 보이지 않는다. 또렷하게 남기려면 낮춘다(0.40이면 확대율 4 이상에서 읽힌다).
